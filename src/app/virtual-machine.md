...
上述命令会将远程桌面端口暴露在本地 5901 端口。

<aside class="note info">
<div class="title">说明</div>

1. 虚拟机名称可以从 **操作 > 查看信息** 中获取，参考[连接虚拟机终端](#连接虚拟机终端)。
2. 上述命令需要使用 API Key，其生成方式参考 [API Key](../security/account.md#api-key)。
...
Mac 系统中自带 VNC Client 功能：打开 Finder，从菜单栏点击 **Go -> Connect to Server…**（或在 Finder 中使用快捷键 **Command + K**）。
...
安装一个 Virtual Machine，选择合适的基础镜像：
...
待 Virtual Machine 就绪后，[连接到虚拟机终端](#连接虚拟机终端)，本节后续操作都是在虚拟机终端中进行。

用户可以在虚拟机中执行安装软件、修改系统配置等操作，只要这些操作的最终修改发生在系统盘中，则可以被记录在新镜像中。

#### 镜像上传

安装一个 JupyterLab App，绑定虚拟机所使用的 PVC（PVC 名称可以从 Virtual Machine App 的详情页面获取）。

待 JupyterLab App 就绪后，进入 JupyterLab UI，在工作目录下创建 Dockerfile 文件，内容为：
...
使用如下 YAML 创建 ImageBuilder 构建镜像：
...
2. lightdm 则是一种轻量化的桌面管理服务

用户可以根据需要进行选择。

<aside class="note info">
<div class="title">apt 换源</div>

在国内，通常无法访问 apt 默认源，执行以下代码进行 apt 换源：
