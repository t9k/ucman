...
<div class="title"> 平台管理员、组织管理员和项目管理员的区别 </div>

**平台管理员**拥有 TensorStack AI 平台的管理权限，包括但不限于创建、修改和删除用户、用户组、项目和队列。平台管理员也具有所有项目的项目管理员和成员的设置权限。

**组织管理员**拥有 TensorStack AI 平台中一个组织的管理权限，包括但不限于创建、修改和删除组织内用户、用户组和项目。组织管理员也具有所有同组织项目的项目管理员和成员的设置权限。

**项目管理员**具有一个项目的成员管理权限。每一个项目只能设置一个项目管理员，由平台管理员在创建项目时设置或在创建后修改。

需要说明的是：平台管理员虽然有集群管理的权限，但是在项目的使用上，平台管理员同样要遵循项目授权的规则，即如果平台管理员不是项目的成员，则平台管理员不能使用这个项目。当然，平台管理员可以通过将自己设置为项目成员的方式来使用项目。组织管理员同样如此。

本节所有“普通用户”指的都是除“平台管理员”之外的用户，也包括项目管理员。
...
项目可以被应用到 TensorStack AI 平台的模型构建、模型部署和工作流三个模块，平台管理员可以设置一个项目的使用范围。
